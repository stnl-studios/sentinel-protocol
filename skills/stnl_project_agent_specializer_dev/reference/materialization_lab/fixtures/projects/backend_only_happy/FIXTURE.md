---
fixture_id: projects/backend_only_happy
fixture_type: project_positive
status: complete
scenario: backend-only-happy
purpose: Backend-only happy project package
dev_only: true
no_real_write: true
source_model:
  kernel_source:
...
lazy_load_expectation:
  demand_type: backend-only-happy
  activated_modules:
...
target_safety:
  ...
  simulated_target_paths:
    - projects/backend_only_happy/simulated_target/docs/INDEX.md
    - projects/backend_only_happy/simulated_target/package.manifest
...
# Fixture Intent
Backend-only happy project package.
...
Selected agents: orchestrator, planner, validation-eval-designer, execution-package-designer, coder-backend, validation-runner, reviewer, finalizer, resync. Resync is included where this is a future package fixture; invocation remains demand-gated by kernels.
