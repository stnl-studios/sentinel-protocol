---
fixture_id: projects/frontend_only_happy
fixture_type: project_positive
status: complete
scenario: frontend-only-happy
purpose: Frontend-only happy project package
dev_only: true
no_real_write: true
source_model:
  kernel_source:
...
lazy_load_expectation:
  demand_type: frontend-only-happy
  activated_modules:
...
target_safety:
  ...
  simulated_target_paths:
    - projects/frontend_only_happy/simulated_target/docs/INDEX.md
    - projects/frontend_only_happy/simulated_target/package.manifest
...
# Fixture Intent
Frontend-only happy project package.
...
Selected agents: orchestrator, planner, validation-eval-designer, execution-package-designer, designer, coder-frontend, validation-runner, reviewer, finalizer, resync. Resync is included where this is a future package fixture; invocation remains demand-gated by kernels.
