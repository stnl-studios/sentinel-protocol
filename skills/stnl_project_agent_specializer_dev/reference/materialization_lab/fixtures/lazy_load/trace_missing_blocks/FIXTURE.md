---
fixture_id: lazy_load/trace_missing_blocks
fixture_type: lazy_load_negative
status: complete
scenario: missing lazy load trace
purpose: Material decision without lazy-load trace must block
dev_only: true
no_real_write: true
source_model:
  kernel_source:
...
lazy_load_expectation:
  demand_type: missing lazy load trace
  activated_modules:
...
target_safety:
  ...
  simulated_target_paths:
    - lazy_load/trace_missing_blocks/trace.yaml
...
# Fixture Intent
Material decision without lazy-load trace must block.
