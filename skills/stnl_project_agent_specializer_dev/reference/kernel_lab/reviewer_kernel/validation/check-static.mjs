...
const KERNEL_PREFIX =
  'skills/stnl_project_agent_specializer_dev/reference/kernel_lab/reviewer_kernel';
...
const patternForbiddenClaims = Object.freeze([
  ...
  { claimName: 'REVIEW_CLEAR without artifact or diff', family: 'output-shape', blocker: 'BLOCKED_RV_REVIEW_CLEAR_SHAPE_INVALID', pattern: /\bREVIEW_CLEAR\b[\s\S]{0,80}\b(?:may|can|could|should|allowed|permitted|emitted|available)\b[\s\S]{0,120}\b(?:without|absent|missing|no)\b[\s\S]{0,80}\b(?:artifact|diff)\b|\bREVIEW_CLEAR\b[\s\S]{0,80}\b(?:may|can|could|should|allowed|permitted|emitted|available)\b[\s\S]{0,120}\b(?:artifact|diff)\b[\s\S]{0,80}\b(?:absent|missing)\b/i },
  { claimName: 'REVIEW_CLEAR with unresolved material risk', family: 'output-shape', blocker: 'BLOCKED_RV_MATERIAL_RISK_NOT_REVIEW_RISK', pattern: /\bREVIEW_CLEAR\b[\s\S]{0,80}\b(?:may|can|could|should|permitted|emitted)\b[\s\S]{0,120}\bunresolved\s+material\s+risk\b|\bunresolved\s+material\s+risk\b[\s\S]{0,120}\bREVIEW_CLEAR\b/i },
  { claimName: 'REVIEW_CLEAR with CORRECTION PACK', family: 'output-shape', blocker: 'BLOCKED_RV_CORRECTION_PACK_INVALID', pattern: /\bREVIEW_CLEAR\b[\s\S]{0,80}\b(?:may|can|could|should|permitted|emitted)\b[\s\S]{0,80}\bwith\b[\s\S]{0,40}\bCORRECTION\s+PACK\b|\bCORRECTION\s+PACK\b[\s\S]{0,120}\b(?:may|can|could|should|permitted|emitted)\b[\s\S]{0,80}\bwith\b[\s\S]{0,40}\bREVIEW_CLEAR\b/i },
  { claimName: 'REVIEW_RISK for aesthetic preference', family: 'output-shape', blocker: 'BLOCKED_RV_OPINION_BLOCKS_CLOSURE', pattern: /\bREVIEW_RISK\b[\s\S]{0,80}\b(?:may|can|could|should|allowed|permitted|used)\b[\s\S]{0,120}\baesthetic\s+preference\b|\baesthetic\s+preference\b[\s\S]{0,120}\b(?:may|can|could|should|allowed|permitted|used)\b[\s\S]{0,80}\bREVIEW_RISK\b/i },
...
function checkSectionAwareAnchors(docs) {
  ...
  requirePatterns(
    ...
    [
      ...
      ['REVIEW_CLEAR terminal signal', /\bREVIEW_CLEAR\b/],
      ['REVIEW_RISK terminal signal', /\bREVIEW_RISK\b/],
      ['exactly one CORRECTION PACK', /\bexactly one\b[\s\S]*\bCORRECTION PACK\b/i],
      ['mutual exclusion', /\bmutually exclusive\b[\s\S]*\bREVIEW_CLEAR\b[\s\S]*\bREVIEW_RISK\b/i],
      ['REVIEW_CLEAR structural adherence', /\bREVIEW_CLEAR\b[\s\S]*\bstructural adherence\b/i],
      ['REVIEW_RISK material risk', /\bREVIEW_RISK\b[\s\S]*\bmaterial\b[\s\S]*\brisk\b/i],
    ],
  ...
  requirePatterns(
    ...
    [
      ...
      ['required advisory output', /\bmay emit\b[\s\S]*\bREVIEW_CLEAR\b[\s\S]*\bREVIEW_RISK\b[\s\S]*\bCORRECTION PACK\b/i],
    ],
    'README Scope Limits',
  );
  ...
  requirePatterns(
    ...
    [
      ['short delta-only', /\bshort\b[\s\S]*\bdelta-only\b/i],
      ['REVIEW_CLEAR', /\bREVIEW_CLEAR\b/],
      ['REVIEW_RISK', /\bREVIEW_RISK\b/],
  ...
  requirePatterns(
    ...
    [
      ...
      ['exclusive with REVIEW_CLEAR REVIEW_RISK', /\bmutually exclusive\b[\s\S]*\bREVIEW_CLEAR\b[\s\S]*\bREVIEW_RISK\b/i],
    ],
    'RV-GATE-008',
  );
}
...
function checkOutputContract(docs) {
  ...
  requirePatterns(
    output,
    [
      ['allowed terminal signals only REVIEW_CLEAR REVIEW_RISK', /Allowed review-specific terminal signals:[\s\S]*-\s+`?REVIEW_CLEAR`?[\s\S]*-\s+`?REVIEW_RISK`?/i],
      ['allowed non terminal exactly one correction pack', /Allowed non-terminal output:[\s\S]*exactly one[\s\S]*CORRECTION PACK/i],
      ['correction mutually exclusive', /CORRECTION PACK[\s\S]*mutually exclusive[\s\S]*REVIEW_CLEAR[\s\S]*REVIEW_RISK/i],
      ['REVIEW_CLEAR requires adherence and no unresolved material risk', /REVIEW_CLEAR[\s\S]*structural adherence[\s\S]*(?:no\s+)?unresolved\s+material structural risk/i],
      ['REVIEW_RISK requires material risk drift inference scope or inability', /REVIEW_RISK[\s\S]*(?:material structural risk|boundary|contract drift|unauthorized inference|scope expansion|inability to judge)/i],
    ],
    'Output contract',
  );
  requirePatterns(
...
function checkReadingContract(docs) {
  ...
  requirePatterns(
    ...
    [
      ...
      ['honest risk when bounded reading insufficient', /cannot support honest semantic review[\s\S]*REVIEW_RISK/i],
