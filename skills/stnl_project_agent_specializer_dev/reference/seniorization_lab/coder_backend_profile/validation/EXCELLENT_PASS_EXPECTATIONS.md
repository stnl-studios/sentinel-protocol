# coder-backend Modular Senior Profile Excellent Pass Expectations

Future audit target:

`coder-backend Modular Senior Agent Profile: EXCELLENT PASS`
...
- `coder_backend_profile` contains `README.md`, a short `SENIOR_AGENT_PROFILE.md` manifest, exactly four behavior modules under `profile/`, and the three local validation files.
...
- `agent_id` is exactly `coder-backend`.
...
- `coder-backend` remains backend executor under package constraints.
- The profile does not authorize takeover of: replanning, package expansion, frontend/iOS execution, validation, review, finalization, resync.
- `coder_backend_kernel` remains preserved as kernel anchor source without raw kernel dump.
...
- The profile preserves the expected artifact or signal: backend executor READY or BLOCKED handoff.
