# execution-package-designer Modular Senior Profile Excellent Pass Expectations

Future audit target:

`execution-package-designer Modular Senior Agent Profile: EXCELLENT PASS`
...
- `execution_package_designer_profile` contains `README.md`, a short `SENIOR_AGENT_PROFILE.md` manifest, exactly four behavior modules under `profile/`, and the three local validation files.
...
- `agent_id` is exactly `execution-package-designer`.
...
- `execution-package-designer` remains bounded execution package designer.
- The profile does not authorize takeover of: routing, implementation, validation execution, semantic review, finalization, resync.
- `execution_package_designer_kernel` remains preserved as kernel anchor source without raw kernel dump.
...
- The profile preserves the expected artifact or signal: EXECUTION PACKAGE.
