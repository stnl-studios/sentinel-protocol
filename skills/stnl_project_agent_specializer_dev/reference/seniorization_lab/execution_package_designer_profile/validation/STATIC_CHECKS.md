# execution-package-designer Modular Senior Profile Static Checks

These checks are documentary/dev-only. They validate the modular `execution_package_designer_profile` bundle and do not authorize runtime loading, materialization, target writes, productive-skill changes, template changes, GitHub writes, or target repository mutation.
...
| `PROFILE_PARTS_HAVE_AGENT_ID` | Bind part to canonical agent. | Each part declares `agent_id: execution-package-designer`. | Agent id is absent, generic, or wrong. |
| `PROFILE_PARTS_HAVE_PURPOSE` | Require specific purpose. | Each part has a execution-package-designer-specific purpose tied to `execution_package_designer_kernel`. | Purpose is blank, generic, or role-agnostic. |
...
| `KERNEL_ANCHORS_PRESERVED` | Preserve kernel-derived behavior. | Modules preserve `execution_package_designer_kernel` anchors without raw kernel dump. | Kernel anchors disappear, become generic, or are copied wholesale. |
| `ROLE_BOUNDARY_PRESERVED` | Preserve canonical role authority. | The profile keeps `execution-package-designer` as bounded execution package designer. | The profile allows execution-package-designer to take over routing, implementation, validation execution, semantic review, finalization, resync. |
