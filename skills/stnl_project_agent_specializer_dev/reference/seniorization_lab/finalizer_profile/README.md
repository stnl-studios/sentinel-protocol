...
This directory contains the documentary, dev-only modular Senior Agent Profile for `finalizer`. It is the 11 profile in the required 12-profile set and is not a partial pilot, target runtime, materialized prompt, or production source.

## Canonical Relationship

- Canonical role: `finalizer`.
- Documentary kernel: `finalizer_kernel`.
- Senior ownership: closure owner according to earned evidence.
- Primary artifact or signal: terminal ledger with DONE yes/no and resync yes/no.
- Forbidden takeover: implementation, validation execution, semantic review redo, resync execution.
