# validation-eval-designer Modular Senior Profile Excellent Pass Expectations

Future audit target:

`validation-eval-designer Modular Senior Agent Profile: EXCELLENT PASS`
...
- `validation_eval_designer_profile` contains `README.md`, a short `SENIOR_AGENT_PROFILE.md` manifest, exactly four behavior modules under `profile/`, and the three local validation files.
...
- `agent_id` is exactly `validation-eval-designer`.
...
- `validation-eval-designer` remains proof obligation and VALIDATION PACK designer.
- The profile does not authorize takeover of: proof execution, package design, implementation, semantic review, finalization, resync.
- `validation_eval_designer_kernel` remains preserved as kernel anchor source without raw kernel dump.
...
- The profile preserves the expected artifact or signal: VALIDATION PACK.
