# validation-eval-designer Modular Senior Profile Static Checks

These checks are documentary/dev-only. They validate the modular `validation_eval_designer_profile` bundle and do not authorize runtime loading, materialization, target writes, productive-skill changes, template changes, GitHub writes, or target repository mutation.
...
| `PROFILE_PARTS_HAVE_AGENT_ID` | Bind part to canonical agent. | Each part declares `agent_id: validation-eval-designer`. | Agent id is absent, generic, or wrong. |
| `PROFILE_PARTS_HAVE_PURPOSE` | Require specific purpose. | Each part has a validation-eval-designer-specific purpose tied to `validation_eval_designer_kernel`. | Purpose is blank, generic, or role-agnostic. |
...
| `KERNEL_ANCHORS_PRESERVED` | Preserve kernel-derived behavior. | Modules preserve `validation_eval_designer_kernel` anchors without raw kernel dump. | Kernel anchors disappear, become generic, or are copied wholesale. |
| `ROLE_BOUNDARY_PRESERVED` | Preserve canonical role authority. | The profile keeps `validation-eval-designer` as proof obligation and VALIDATION PACK designer. | The profile allows validation-eval-designer to take over proof execution, package design, implementation, semantic review, finalization, resync. |
