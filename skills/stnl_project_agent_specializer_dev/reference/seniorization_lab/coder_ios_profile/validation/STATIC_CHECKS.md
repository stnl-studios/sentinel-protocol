# coder-ios Modular Senior Profile Static Checks

These checks are documentary/dev-only. They validate the modular `coder_ios_profile` bundle and do not authorize runtime loading, materialization, target writes, productive-skill changes, template changes, GitHub writes, or target repository mutation.
...
| `PROFILE_PARTS_HAVE_AGENT_ID` | Bind part to canonical agent. | Each part declares `agent_id: coder-ios`. | Agent id is absent, generic, or wrong. |
| `PROFILE_PARTS_HAVE_PURPOSE` | Require specific purpose. | Each part has a coder-ios-specific purpose tied to `coder_ios_kernel`. | Purpose is blank, generic, or role-agnostic. |
...
| `KERNEL_ANCHORS_PRESERVED` | Preserve kernel-derived behavior. | Modules preserve `coder_ios_kernel` anchors without raw kernel dump. | Kernel anchors disappear, become generic, or are copied wholesale. |
| `ROLE_BOUNDARY_PRESERVED` | Preserve canonical role authority. | The profile keeps `coder-ios` as native iOS executor under package constraints. | The profile allows coder-ios to take over replanning, package expansion, frontend/backend execution, validation, review, finalization, resync. |
