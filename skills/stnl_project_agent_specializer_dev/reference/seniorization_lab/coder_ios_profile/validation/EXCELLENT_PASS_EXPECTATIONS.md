# coder-ios Modular Senior Profile Excellent Pass Expectations

Future audit target:

`coder-ios Modular Senior Agent Profile: EXCELLENT PASS`
...
- `coder_ios_profile` contains `README.md`, a short `SENIOR_AGENT_PROFILE.md` manifest, exactly four behavior modules under `profile/`, and the three local validation files.
...
- `coder-ios` remains native iOS executor under package constraints.
- The profile does not authorize takeover of: replanning, package expansion, frontend/backend execution, validation, review, finalization, resync.
- `coder_ios_kernel` remains preserved as kernel anchor source without raw kernel dump.
...
- The profile preserves the expected artifact or signal: iOS executor READY or BLOCKED handoff.
