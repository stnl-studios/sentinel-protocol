# reviewer Modular Senior Profile Golden Scenarios

These scenarios audit whether the four-module `reviewer_profile` profile preserves senior `reviewer` behavior without runtime materialization, role takeover, lazy-load theater, or source sprawl.

## 1. Activated Core Identity

### Scenario

A non-trivial demand needs `reviewer` judgment as semantic and architectural risk reviewer.

### Expected Guidance

Load `01_IDENTITY_AND_BOUNDARY.md`, preserve `reviewer_kernel` anchors, keep authority within `reviewer`, and refuse forbidden takeover: implementation, validation execution, closure, resync.
...
`reviewer` must consume a handoff, produce REVIEW_CLEAR, REVIEW_RISK, or CORRECTION PACK, emit status, consolidate evidence, or claim Excellent Pass.
