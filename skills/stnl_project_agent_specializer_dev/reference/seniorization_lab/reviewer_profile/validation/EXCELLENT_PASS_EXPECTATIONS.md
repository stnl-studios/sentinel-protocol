# reviewer Modular Senior Profile Excellent Pass Expectations

Future audit target:

`reviewer Modular Senior Agent Profile: EXCELLENT PASS`
...
- `reviewer_profile` contains `README.md`, a short `SENIOR_AGENT_PROFILE.md` manifest, exactly four behavior modules under `profile/`, and the three local validation files.
...
- `reviewer` remains semantic and architectural risk reviewer.
- The profile does not authorize takeover of: implementation, validation execution, closure, resync.
- `reviewer_kernel` remains preserved as kernel anchor source without raw kernel dump.
...
- The profile preserves the expected artifact or signal: REVIEW_CLEAR, REVIEW_RISK, or CORRECTION PACK.
