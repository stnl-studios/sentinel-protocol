...
const forbiddenDocumentClaims = Object.freeze([
  ...
  { name: 'untrusted source of truth', blocker: 'BLOCKED_RV_UNTRUSTED_SOURCE_OF_TRUTH', pattern: /\b(?:scratchpads?|workspaceStorage|chat-session-resources|content\.txt|runtime temp paths?)\b[\s\S]{0,120}\b(?:source of truth|canonical(?:\s+(?:source|truth))?|authoritative(?:\s+(?:source|truth))?|trusted(?:\s+source)?|accepted\s+(?:truth|source\s+of\s+truth))\b|\b(?:source of truth|canonical(?:\s+(?:source|truth))?|authoritative(?:\s+(?:source|truth))?|trusted(?:\s+source)?|accepted\s+(?:truth|source\s+of\s+truth))\b[\s\S]{0,120}\b(?:scratchpads?|workspaceStorage|chat-session-resources|content\.txt|runtime temp paths?)\b/i },
  { name: 'productive template fallback', blocker: 'BLOCKED_RV_PRODUCTIVE_TEMPLATE_FALLBACK', pattern: /\b(?:productive\s+)?templates?\b[\s\S]{0,120}\b(?:fallback|backs?\s+up\s+missing\s+snapshot|replaces?\s+missing\s+snapshot)\b|\b(?:fallback|backs?\s+up\s+missing\s+snapshot|replaces?\s+missing\s+snapshot)\b[\s\S]{0,120}\b(?:productive\s+)?templates?\b/i },
]);
