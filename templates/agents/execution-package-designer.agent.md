...
When reading project, SPEC, or context files, first check whether a `File Purpose Header` exists near the top. Use `read_when`, `do_not_use_for`, `canonical_source_for`, `canonical_source_not_for`, and `token_policy` to find canonical sources for scope, slice, decisions, constraints, and readiness before compiling package fields. Respect `do_not_use_for`: do not extract execution plans, file paths, commands, work packages, `OWNED_PATHS`, anchors, sequencing, or coder instructions from files that identify themselves as non-canonical for those purposes. If a header points package-critical truth elsewhere, open that canonical file; if it is absent or inconsistent, emit `BLOCKED` under the existing package-readiness rules. If no header exists, continue with normal legacy reading and never treat absence as an error. Headers may route reading, but they must not fill `OWNED_PATHS`, commands, work packages, acceptance checks, or block conditions, and they never authorize a package when the SPEC or slice is blocked or deferred for planning.
